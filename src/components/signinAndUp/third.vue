<template>
  <div class="main">
    ...
    <Button type="info" @click="tj3" style="margin-left: 39%; font-size: 17px;width:25% ; margin-top:3%;background: #6a4f90;border-color: #6a4f90;height:40px;">提交
    </Button>
  ...
  export default {
    ...
    methods: {
      getpassword2: function () {
        ...
        this.grtr12 = false
        this.grtr13 = false
        this.passwordwarning1 = '建议使用数字、字母、符号两种及以上的组合，6-20个字符'
      },
      losepassword2: function () {
        if (this.password2.length > 5 && this.password2.length < 21) {
          if (this.password2 === this.repassword2) {
            ...
            this.passwordwarning1 = ''
            this.isrepasswordcolor1 = true
            this.repasswordwarning1 = '两次密码输入不一致'
          }
      ...
      tj3: function () {
        if (this.password2.length > 5 && this.password2.length < 21 && this.password2 === this.repassword2) {
        ...
        } else if (this.password2.length < 6 || this.password2.length > 20) {
          this.$Notice.error({ title: '密码设置的长度不符合要求'})
        } else {
          this.$Notice.error({ title: '两次密码不一致'})
        }
      }
    }
  }
